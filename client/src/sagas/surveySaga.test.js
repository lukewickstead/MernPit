...
describe('When on the existing supporter page', () => {
  describe('and the user the calls the supporter page next button', () => {
    it('should navigate to /Name and set the correct data into state', () => {
      ...
      expect(saga.next().value).toEqual(put(putExistingSupporterIntoStateAction(stubbedIsExistingSupporter)));
      expect(saga.next().value).toEqual(put(push(URL__NAME)));
  ...
  describe('and the user calls the supporter page back button', () => {
    it('the user is redirected to /Home', () => {
      ...
      expect(saga.next().value).toEqual(put(push(URL__HOME)));
      expect(saga.next().value).toBeUndefined();
    });
  });
});
...
describe('When on the applicant name page', () => {
  describe('and the user calls the applicant name next button', () => {
    it('should navigate to /Phone and set the correct data into state', () => {
      ...
      expect(saga.next().value).toEqual(put(putApplicantNameIntoStateAction(stubbedFirstName, stubbedLastName)));
      expect(saga.next().value).toEqual(put(push(URL__PHONE_NUMBERS)));
      expect(saga.next().value).toBeUndefined();
    });
  });
  ...
  describe('and the user calls the applicant page back button', () => {
    it('the user is redirected to /ExistingSupporter', () => {
      ...
      expect(saga.next().value).toEqual(put(push(URL__EXISTING_SUPPORTER)));
      expect(saga.next().value).toBeUndefined();
    });
  });
});
...
describe('When on the applicant phone page', () => {
  describe('and the user calls the applicant phone next button', () => {
    it('should navigate to /Email and set the correct data into state', () => {
      ...
      expect(saga.next().value).toEqual(put(putApplicantPhoneNumberIntoStateAction(stubbedOfficeNo, stubbedMobileNo)));
      expect(saga.next().value).toEqual(put(push(URL__EMAIL)));
      expect(saga.next().value).toBeUndefined();
    });
  });
...
describe('When on the applicant email page', () => {
  describe('and the user calls the email page next button', () => {
    it('should navigate to Experience and set the correct data into state', () => {
      ...
      expect(saga.next().value).toEqual(put(putApplicantEmailIntoStateAction(stubbedEmail)));
      expect(saga.next().value).toEqual(put(push(URL__SUPPORTER_EXPERIENCE)));
      expect(saga.next().value).toBeUndefined();
    });
  });
...
describe('When on the supporters experience page', () => {
  describe('and the user calls the supporters experience page next button', () => {
    it('should set the correct data into state', () => {
      ...
      expect(saga.next().value).toEqual(put(putSupporterExperienceIntoStateAction(stubbedYearsSupporting, stubbedMatchesWatched, stubbedShirtsOwned)));
      expect(saga.next().value).toEqual(put(push(URL__SUMMARY)));
      expect(saga.next().value).toBeUndefined();
    });
  });
...
describe('when on the summary page', () => {
  describe('and they select the next button', () => {
    it('should submit the survey and navigate to /Confirmation', () => {
      ...
      expect(saga.next().value).toEqual(put(putIsBusyAction(true)));
      expect(saga.next().value).toEqual(select(getSurveyDetailsFromState));
      expect(saga.next(stubbedSurveyDetails).value).toEqual(call(postSurvey, stubbedSurveyDetails));
      expect(saga.next().value).toEqual(put(push(URL__CONFIRMATION)));
      expect(saga.next().value).toEqual(put(putIsBusyAction(false)));
      expect(saga.next().value).toBeUndefined();
      expect(saga.next().value).toBeUndefined();
    });
    ...
    describe('and an error is reported posting the survey', () => {
      it('should log the error and navigate to /Error', () => {
        ...
        expect(saga.next().value).toEqual(select(getSurveyDetailsFromState));
        expect(saga.next(stubbedSurveyDetails).value).toEqual(call(postSurvey, stubbedSurveyDetails));
        expect(saga.throw(stubbedPostSurveyResponse).value).toEqual(call(getErrorMessageFromServerResponseError, stubbedPostSurveyResponse));
        expect(saga.next(stubbedPostSurveyResponse.data).value).toEqual(call(consoleError, `Could not submit survey: ${stubbedPostSurveyResponse.data}`));
        expect(saga.next().value).toEqual(put(push(URL__ERROR)));
        expect(saga.next().value).toEqual(put(putIsBusyAction(false)));
        expect(saga.next().value).toBeUndefined();
      });
    });
...
describe('when calling watchSurveyAggregateSaga saga', () => {
  it('can watch for all required sagas', () => {
    ...
    expect(saga.next().value).toEqual(takeEvery(EXISTING_SUPPORTER__NEXT, applicantExistingSupporterNextSaga));
    expect(saga.next().value).toEqual(takeEvery(EXISTING_SUPPORTER__BACK, applicantExistingSupporterBackSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_NAME__NEXT, applicantNameNextSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_NAME__BACK, applicantNameBackSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_PHONE_NOS__NEXT, applicantPhoneNextSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_PHONE_NOS__BACK, applicantPhoneBackSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_EMAIL__NEXT, applicantEmailNextSaga));
    expect(saga.next().value).toEqual(takeEvery(APPLICANT_EMAIL__BACK, applicantEmailBackSaga));
    expect(saga.next().value).toEqual(takeEvery(SUPPORTER_EXPERIENCE__NEXT, supporterExperienceNextSaga));
    expect(saga.next().value).toEqual(takeEvery(SUPPORTER_EXPERIENCE__BACK, supporterExperienceBackSaga));
    expect(saga.next().value).toEqual(takeEvery(SUMMARY__NEXT, summaryPageNextSaga));
    expect(saga.next().value).toEqual(takeEvery(SUMMARY__BACK, summaryPageBackSaga));
    expect(saga.next().value).toBeUndefined();
  });
});
